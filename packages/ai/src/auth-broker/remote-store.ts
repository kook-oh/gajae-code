...
export class RemoteAuthCredentialStore implements AuthCredentialStore {
	...
	replaceAuthCredentialsForProvider(_provider: string, _credentials: AuthCredential[]): StoredAuthCredential[] {
		throw new Error(
			"RemoteAuthCredentialStore is read-only on the client. Use `worx auth-broker login <provider>` to mutate credentials.",
		);
	}
	...
	deleteAuthCredentialsForProvider(_provider: string, _disabledCause: string): void {
		throw new Error(
			"RemoteAuthCredentialStore is read-only on the client. Use `worx auth-broker logout <provider>` to mutate credentials.",
		);
	}
