# @bworx-io/worx-agent-core

Stateful agent with tool execution and event streaming. Built on `@bworx-io/worx-ai`.

## Installation

```bash
npm install @bworx-io/worx-agent-core
```
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@bworx-io/worx-ai`).
...
```typescript
import { Agent, streamProxy } from "@bworx-io/worx-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@bworx-io/worx-agent-core";
