...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@bworx-io/worx-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.worx/`), plus
//    the legacy non-XDG `~/.worx/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.worx/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/gjc/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.worx` when
...
// symlinks `~/.worx/agent/extensions/foo.ts` to a dotfiles repo would see the
