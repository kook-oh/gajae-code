...
import { type ContextFile, contextFileCapability } from "@bworx-io/worx-code/capability/context-file";
...
describe("contextFileCapability.key", () => {
	...
	test("user-level files share the same key regardless of depth", () => {
		const a = makeContextFile({ path: "/home/user/.worx/agent/AGENTS.md", level: "user" });
	...
	test("user key never collides with any project key", () => {
		const user = makeContextFile({ path: "/home/user/.worx/AGENTS.md", level: "user" });
