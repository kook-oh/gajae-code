...
import { loadCapability } from "@bworx-io/worx-code/capability";
import { clearCache as clearFsCache } from "@bworx-io/worx-code/capability/fs";
import {
...
import "@bworx-io/worx-code/discovery/claude-plugins";
import type { Skill } from "@bworx-io/worx-code/capability/skill";
import type { SlashCommand } from "@bworx-io/worx-code/capability/slash-command";
...
describe("listClaudePluginRoots", () => {
	...
	test("parses plugin with user scope", async () => {
		const pluginsDir = path.join(tempDir, ".worx", "plugins");
...
describe("discoverAgents plugin precedence", () => {
	...
	test("prefers project-scoped plugin agent over user-scoped plugin agent", async () => {
		const pluginRegistryDir = path.join(tempDir, ".worx", "plugins");
