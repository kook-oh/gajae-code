 ...
 * from both `~/.worx/agent/RULES.md` (user) and the nearest `.worx/RULES.md`
...
import { getCapability } from "@bworx-io/worx-code/capability";
import { clearCache } from "@bworx-io/worx-code/capability/fs";
import { type Rule, ruleCapability } from "@bworx-io/worx-code/capability/rule";
import type { LoadContext } from "@bworx-io/worx-code/capability/types";
// Register all discovery providers as a side effect.
import "@bworx-io/worx-code/discovery";
...
test("user ~/.worx/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".worx", "agent", "RULES.md"),
...
test("project .worx/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".worx", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".worx", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".worx", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".worx", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .worx/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".worx", "agent", "rules", "other.md"), "# Unrelated rule\n");
