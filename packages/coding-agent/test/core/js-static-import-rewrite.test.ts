...
describe("rewriteImports", () => {
	it("rewrites a top-level default import", () => {
		...
		expect(out).toContain('await __worx_import__("bar")');
	...
	it("rewrites destructured named imports with renames", () => {
		...
		expect(out).toContain('await __worx_import__("pkg")');
	...
	it("rewrites namespace imports", () => {
		...
		expect(out).toContain('const ns = await __worx_import__("pkg")');
	});
	...
	it("rewrites side-effect-only imports", () => {
		...
		expect(out).toContain('await __worx_import__("polyfill")');
	});
	...
	it("preserves import attributes via the dynamic import options bag", () => {
		...
		expect(out).toContain('await __worx_import__("./d.json", { with: { type: "json" } })');
	...
	it("rewrites bare dynamic import() so its specifier resolves against the session cwd", () => {
		...
		expect(out).toContain('await __worx_import__("./foo.ts")');
	...
	it("rewrites dynamic import() with an options bag (passes options through unchanged)", () => {
		...
		expect(out).toContain('__worx_import__("./d.json", { with: { type: "json" } })');
	});
	...
	it("rewrites nested and chained dynamic import() calls", () => {
		...
		expect(out).toContain('__worx_import__("./a.ts")');
		expect(out).toContain('__worx_import__("./b.ts")');
	...
	it("rewrites dynamic import() with a non-literal specifier", () => {
		...
		expect(out).toContain("__worx_import__(spec)");
	});
	...
	it("does not rewrite import statements embedded in template literals (the bug)", () => {
		...
		expect(out).not.toContain("await __worx_import__(");
	});
	...
	it("does not rewrite import statements inside block comments", () => {
		...
		expect(out).not.toContain('await __worx_import__("bar")');
	});
	...
	it("rewrites real top-level imports while leaving template-embedded look-alikes alone", () => {
		...
		const out = rewriteImports(code);
		expect(out).toContain('await __worx_import__("alpha")');
		expect(out).toContain('await __worx_import__("gamma")');
		expect(out).not.toContain('await __worx_import__("beta")');
	...
	it("captures the final expression even when trailing empty statements follow", () => {
		...
		expect(wrapped.source).toContain("__worx_set_final_expr__((await Promise.resolve(1)))");
	});
});
