...
} from "@bworx-io/worx-code/worx-runtime/ralplan-runtime";
import {
	WORX_RALPLAN_ARTIFACT_ENV,
	WORX_RESTRICTED_ROLE_AGENT_BASH_ENV,
} from "@bworx-io/worx-code/worx-runtime/restricted-role-agent-bash";
import {
	activeEntryPath,
	activeSnapshotPath,
	modeStatePath,
...
describe("native worx ralplan runtime — consensus handoff", () => {
...
describe("native worx ralplan runtime — --write artifact path", () => {
...
describe("native worx ralplan runtime — run-state phase coherence", () => {
...
describe("native worx ralplan runtime — duplicate --write guard", () => {
...
describe("native worx ralplan runtime — persisted role-agent state", () => {
...
describe("native worx ralplan runtime — post-clear re-activation (#644)", () => {
	...
	it("re-asserts active:true and resets phase out of terminal lock when a new run_id is written after a clear", async () => {
		...
		// Simulate `worx state ralplan clear`: active -> false, phase -> complete.
...
describe("ralplan automatic handoff admission (#3398)", () => {
	...
	it("rejects malformed project settings rather than falling through to user settings", async () => {
		...
		try {
			...
			await fs.writeFile(
				...
				JSON.stringify({ worx: { ralplan: { autoHandoff: "ultragoal" } } }),
				"utf-8",
			);
	...
	it("rejects an invalid configured automatic handoff target", async () => {
		...
		const projectPath = path.join(root, ".worx", "settings.json");
		await fs.mkdir(path.dirname(projectPath), { recursive: true });
		await fs.writeFile(projectPath, JSON.stringify({ worx: { ralplan: { autoHandoff: "later" } } }), "utf-8");

		await expect(resolveRalplanAutoHandoff(root)).rejects.toThrow(
			`invalid ralplan settings at ${projectPath}: expected worx.ralplan.autoHandoff to be one of off, ultragoal, team`,
		);
	});
	it("rejects invalid final admission settings before writing final artifacts", async () => {
		...
		await fs.writeFile(
			...
			JSON.stringify({ worx: { ralplan: { autoHandoff: "later" } } }),
			"utf-8",
		);
	...
	it("resolves ultragoal and a usable team target from project settings", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { autoHandoff: "team" } } }),
			"utf-8",
		);
		expect(
	...
	it("overlays a later durable PLANNING-STUCK marker on final dedupe", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { autoHandoff: "ultragoal", maxIterations: 1 } } }),
			"utf-8",
		);
	...
	it("uses the final ledger admission after state loss and settings changes", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { autoHandoff: "off" } } }),
			"utf-8",
		);
...
describe("ralplan consensus iteration cap (#3165)", () => {
	...
	it("honors project settings maxIterations=2 and resets budget on new run_id", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { maxIterations: 2 } } }),
			"utf-8",
		);
...
describe("ralplan crash-gap dedupe repair", () => {
	...
	it("does not consume a raised-budget second slot while repairing a crash gap", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { maxReviewPassesPerLane: 2 } } }),
			"utf-8",
		);
		await fs.mkdir(runDir, { recursive: true });
...
describe("ralplan review lane budget settings", () => {
	it("resolves nested and flat settings with project-over-user precedence", async () => {
		...
		try {
			...
			await fs.mkdir(path.join(root, ".worx"), { recursive: true });
			const projectPath = path.join(root, ".worx", "settings.json");
			await fs.writeFile(projectPath, JSON.stringify({ worx: { ralplan: { maxReviewPassesPerLane: 3 } } }), "utf-8");
			expect(await resolveRalplanMaxReviewPassesPerLane(root)).toEqual({
				maxReviewPassesPerLane: 3,
				source: projectPath,
			});

			await fs.writeFile(projectPath, JSON.stringify({ "worx.ralplan.maxReviewPassesPerLane": 4 }), "utf-8");
			expect(await resolveRalplanMaxReviewPassesPerLane(root)).toEqual({
	...
	it("rejects an invalid present project value rather than defaulting", async () => {
		...
		const projectPath = path.join(root, ".worx", "settings.json");
		await fs.mkdir(path.dirname(projectPath), { recursive: true });
		await fs.writeFile(projectPath, JSON.stringify({ worx: { ralplan: { maxReviewPassesPerLane: 99 } } }), "utf-8");
...
describe("ralplan HUD lane verdict carriage", () => {
	...
	it("uses the resolved per-lane budget as the review-pass denominator", async () => {
		...
		await fs.writeFile(
			path.join(root, ".worx", "settings.json"),
			JSON.stringify({ worx: { ralplan: { maxReviewPassesPerLane: 3 } } }),
			"utf-8",
		);
		expect((await writeRalplanArtifact(root, runId, "planner", 1, "# plan")).status).toBe(0);
		expect(
