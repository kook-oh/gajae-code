...
import { reconcileWorkflowSkillState } from "@bworx-io/worx-code/worx-runtime/state-runtime";
import {
	validateCompletionReceipt,
	verifyUltragoalDurableCompletionState,
} from "@bworx-io/worx-code/worx-runtime/ultragoal-guard";
import {
	addUltragoalSubgoal,
...
const NON_COMPUTER_TEST_PATH = "packages/coding-agent/test/worx-runtime/ultragoal-runtime.test.ts";
...
function batchChangeSetPaths(): Array<{ path: string; status: string }> {
	return [
		{ path: "packages/coding-agent/src/worx-runtime/ultragoal-runtime.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/worx-runtime/ultragoal-guard.ts", status: "unknown" },
		{ path: "packages/coding-agent/src/worx-runtime/ultragoal-receipt-freshness.ts", status: "unknown" },
		{ path: "packages/coding-agent/test/worx-runtime/ultragoal-runtime.test.ts", status: "unknown" },
...
describe("native GJC ultragoal runtime", () => {
	...
	it("validation batch deferred: explicit paths must exactly match the computed cumulative diff", async () => {
		const root = await batchTempDir();
		const actualPath = "packages/coding-agent/src/worx-runtime/ultragoal-runtime.ts";
		const secondPath = "packages/coding-agent/test/worx-runtime/ultragoal-runtime.test.ts";
	...
	it("review cohort: a later generation must be delta-only over a new frozen source", async () => {
		...
		const secondGeneration = (overrides: Record<string, unknown>) => {
			...
			gate.iteration.reviewCohort = {
				...
				deltaOnly: true,
				deltaPaths: ["packages/coding-agent/src/worx-runtime/ultragoal-runtime.ts"],
	...
	it("validation batch deferred: uses CI changed paths when git diff is unavailable", async () => {
		...
		try {
			...
			uncovered.deferredToBatch.changeSet.paths = [
				{ path: "packages/coding-agent/src/worx-runtime/ultragoal-runtime.ts", status: "modified" },
			];
	...
	it("does not require computer-use adversarial cases for prompt-only wording changes", async () => {
		...
		await validateExecutorQaRedTeamEvidenceForReview(root, webExecutorQa(), {
			...
			changeSet: {
				...
				paths: [
					{
						path: "packages/coding-agent/src/defaults/worx/skills/ultragoal/SKILL.md",
						status: "modified",
					},
				],
			},
...
describe("ultragoal @goal decomposition", () => {
	...
	it("reconciles completed runs with mode-state and HUD active-state", async () => {
		...
		expect(modeState.receipt).toMatchObject({ skill: "ultragoal", owner: "worx-runtime" });
...
describe("ultragoal mode-state + HUD reconciliation (#342)", () => {
	...
	it("stamps reconcile provenance distinguishable from a user write (AC5)", async () => {
		...
		await withSessionId(TEST_SESSION_ID, async () => {
			...
			expect(receipt.owner).toBe("worx-runtime");
