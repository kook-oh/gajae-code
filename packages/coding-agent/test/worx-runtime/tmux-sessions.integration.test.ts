...
import { registerCoordinatorRuntimeStateFinalizer } from ${JSON.stringify(path.resolve(import.meta.dir, "../../src/worx-runtime/session-state-sidecar.ts"))};
...
`,
		...
		await fs.writeFile(
			supervisorScript,
			`import { writeFile } from "node:fs/promises";
import { runManagedOwnerSupervisor } from ${JSON.stringify(path.resolve(import.meta.dir, "../../src/worx-runtime/managed-owner-supervisor.ts"))};
try {
