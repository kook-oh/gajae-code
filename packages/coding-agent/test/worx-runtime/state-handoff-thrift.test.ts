...
import { appendOrMergeDeepInterviewRound } from "@bworx-io/worx-code/worx-runtime/deep-interview-recorder";
import { runNativeDeepInterviewCommand } from "@bworx-io/worx-code/worx-runtime/deep-interview-runtime";
import { runNativeRalplanCommand } from "@bworx-io/worx-code/worx-runtime/ralplan-runtime";
...
import { createUltragoalPlan, runNativeUltragoalCommand } from "@bworx-io/worx-code/worx-runtime/ultragoal-runtime";
...
describe("CONSUMER/KEY-FIELD MATRIX for compact handoff payloads", () => {
	it("goldens and asserts every preserved consumer key field", async () => {
		...
		expect(scrub(ultragoalHandoff.stdout ?? "")).toMatchInlineSnapshot(`
			...
			gjc-objective=Complete the durable ultragoal plan in .worx/ultragoal/goals.json, including later accepted/appended stories, under the original brief constraints; use .worx/ultragoal/ledger.jsonl as the audit trail.
