...
import { readWorkflowStateJson, runNativeStateCommand } from "@bworx-io/worx-code/worx-runtime/state-runtime";
	...
	it("defaults read output to markdown and keeps --json parseable", async () => {
		...
		await runNativeStateCommand(
			[
				...
				"deep-interview",
				"--input",
				JSON.stringify({ active: true, current_phase: "interviewing", artifact_path: ".worx/specs/draft.md" }),
			],
			root,
		);
		...
		expect(markdown.stdout).toContain(".worx/specs/draft.md");
