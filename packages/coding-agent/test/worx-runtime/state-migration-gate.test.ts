...
import { auditPath, modeStatePath } from "@bworx-io/worx-code/worx-runtime/session-layout";
import { normalizeLegacyState } from "../../src/worx-runtime/state-migrations";
...
describe("G7 worx state migration gate", () => {
	...
	it("migrates tampered state with --force and audits the forced mismatch", async () => {
		await withTempCwd(async cwd => {
			...
			expect(persisted.receipt).toMatchObject({ owner: "worx-state-cli", status: "fresh" });
			...
			expect(mismatch).toMatchObject({ skill: "ralplan", category: "state", owner: "worx-state-cli" });
