...
} from "@bworx-io/worx-code/worx-runtime/deep-interview-state";
...
describe("native worx deep-interview runtime", () => {
	...
	it("fails closed on corrupt deep-interview state unless --force is supplied", async () => {
		...
		expect(forcedState.receipt).toMatchObject({ skill: "deep-interview", owner: "worx-runtime" });
	...
	it("runs an optional bounded trace pre-step before deep-interview questions", async () => {
		const root = await tempDir();
		await fs.mkdir(path.join(root, "packages/coding-agent/src/worx-runtime"), { recursive: true });
		await fs.writeFile(
			path.join(root, "package.json"),
			JSON.stringify({ name: "trace-fixture", scripts: { test: "bun test", check: "bun check" } }),
		);
		await fs.writeFile(
			path.join(root, "packages/coding-agent/src/worx-runtime/deep-interview-runtime.ts"),
