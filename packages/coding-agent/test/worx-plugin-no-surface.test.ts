...
async function installMixedRootRegistry(): Promise<void> {
	...
	const pluginsDir = path.join(tempHome, ".worx", "plugins");
...
async function installProjectSkillFixture(): Promise<void> {
	await fs.mkdir(path.join(tempCwd, ".worx", "worx-plugins"), { recursive: true });
	await fs.cp(
		path.join(fixturesRoot, "valid-skill-plugin"),
		path.join(tempCwd, ".worx", "worx-plugins", "valid-skill-plugin"),
		{ recursive: true },
	);
}
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "worx-plugin-home-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "worx-plugin-nosurface-"));
	agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "worx-plugin-nosurface-agent-"));
