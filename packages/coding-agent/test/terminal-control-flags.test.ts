...
describe("applyTerminalControlFlagsToEnv", () => {
	...
	it("--no-pty sets the canonical WORX_ name and the legacy PI_ name", () => {
		const env: NodeJS.ProcessEnv = {};
		applyTerminalControlFlagsToEnv({ noPty: true, noTitle: false }, env);
		expect(env.WORX_NO_PTY).toBe("1");
		expect(env.PI_NO_PTY).toBe("1");
	});

	it("--no-pty overrides a user's WORX_NO_PTY=0 so the flag keeps CLI authority", () => {
		// Regression: the reader resolves GJC-first, so if the flag only set PI_NO_PTY
		// a user's WORX_NO_PTY=0 would silently override the explicit --no-pty.
		const env: NodeJS.ProcessEnv = { WORX_NO_PTY: "0" };
		applyTerminalControlFlagsToEnv({ noPty: true, noTitle: false }, env);
		expect(env.WORX_NO_PTY).toBe("1");
	});

	it("--no-title sets the canonical WORX_ name and the legacy PI_ name", () => {
		const env: NodeJS.ProcessEnv = { WORX_NO_TITLE: "0" };
		applyTerminalControlFlagsToEnv({ noPty: false, noTitle: true }, env);
		expect(env.WORX_NO_TITLE).toBe("1");
	...
	it("--acp mode implies no-title but not no-pty", () => {
		...
		expect(env.WORX_NO_TITLE).toBe("1");
		expect(env.PI_NO_TITLE).toBe("1");
		expect(env.WORX_NO_PTY).toBeUndefined();
	});
});
