...
describe("GJC inshellisense completion spec", () => {
	it("maps command aliases, root flags, option args, and variadic args into Fig shape", async () => {
		const spec = await buildWorxFigSpec(fakeEntries());

		expect(spec.name).toBe("worx");
	...
	it("keeps command-name completion available when optional command metadata cannot load", async () => {
		const spec = await buildWorxFigSpec([
	...
	it("exports the real GJC command surface, aliases, and root launch flags", async () => {
		...
		// the fast-help/completion mirror this spec builds from, so `worx --help` and shell
	...
	it("installs a GJC-only index and spec into an empty target directory", async () => {
		...
		const result = await installWorxInshellisenseSpec(spec, { dir });

		expect(result).toMatchObject({ directory: dir, indexStatus: "created" });
		expect(await Bun.file(path.join(dir, "index.js")).text()).toContain("Generated by worx completion inshellisense");
		expect(await Bun.file(path.join(dir, "worx.js")).text()).toContain("web-search");
	});
	...
	it("refuses to clobber an unrelated inshellisense index without force", async () => {
		...
		await expect(installWorxInshellisenseSpec(await buildWorxFigSpec(fakeEntries()), { dir })).rejects.toThrow(
			/does not list worx/,
		);
	...
	it("overwrites an unrelated index only when force is explicit", async () => {
		...
		const result = await installWorxInshellisenseSpec(await buildWorxFigSpec(fakeEntries()), { dir, force: true });

		expect(result.indexStatus).toBe("overwritten");
		expect(await Bun.file(path.join(dir, "index.js")).text()).toContain('export default ["worx"]');
	});
});
