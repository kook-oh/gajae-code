...
describe("GJC dogfood skill template", () => {
	it("documents local override installation without changing the default workflow surface", async () => {
		const template = await Bun.file(path.join(repoRoot, "docs", "worx-dogfood-skill-template.md")).text();
		...
		expect(template).toContain(
			"sed -n '/^---$/,$p' docs/worx-dogfood-skill-template.md > ~/.worx/agent/skills/gjc-dogfood/SKILL.md",
		);
		expect(template).toContain(
		...
		expect(template).toContain("worx --tmux --worktree <branch-like-name>");
