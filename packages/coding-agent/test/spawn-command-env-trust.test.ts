 ...
 *
 * - `WORX_SDK_SESSION_COMMAND` becomes the broker's session-host `spawn(file, args)`
 * - `WORX_HARNESS_PROCESS_START_COMMAND` becomes the harness `Bun.spawnSync([...command, pid])`
 *
...
const COMMAND_KEYS = ["WORX_SDK_SESSION_COMMAND", "WORX_HARNESS_PROCESS_START_COMMAND"] as const;
...
describe("spawn command env trust boundary", () => {
	...
	it("ignores an SDK session command planted by the project .env", async () => {
		const cwd = projectDir("WORX_SDK_SESSION_COMMAND=/tmp/attacker-host --take-over\n");
	...
	it("ignores a harness process-start command planted by the project .env", async () => {
		const cwd = projectDir('WORX_HARNESS_PROCESS_START_COMMAND=["/tmp/attacker-probe"]\n');
	...
	it("still honors overrides inherited from the launching shell", async () => {
		const resolved = await resolveIn(projectDir(), {
			WORX_SDK_SESSION_COMMAND: "/opt/gjc/session-host --flag",
			WORX_HARNESS_PROCESS_START_COMMAND: '["ps","-o","lstart=","-p"]',
		});
	...
	it("keeps a malformed inherited harness override fatal rather than falling back", async () => {
		const resolved = await resolveIn(projectDir(), { WORX_HARNESS_PROCESS_START_COMMAND: "not json" });
		expect(resolved.processStartCommand).toEqual({ kind: "invalid" });
	});

	it("does not let the project .env override inherited values", async () => {
		const cwd = projectDir("WORX_SDK_SESSION_COMMAND=/tmp/attacker-host\n");
		expect((await resolveIn(cwd, { WORX_SDK_SESSION_COMMAND: "/opt/gjc/session-host" })).sdkSessionCommand).toEqual({
