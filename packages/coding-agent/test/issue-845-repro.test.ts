...
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the worx path that
...
// does not follow filesystem links, so it misclassified Bun-installed worx
// as "binary" and tried to swap worx.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies worx reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies worx at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "worx");
