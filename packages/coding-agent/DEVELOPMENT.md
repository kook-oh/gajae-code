...
- **SDK transport client**: `@bworx-io/worx-bridge-client` owns the standalone transport-only v3 `SdkClient` and its associated types. `src/sdk/client/client.ts` and `@bworx-io/worx-code/sdk` compatibility exports re-export that exact class identity; protocol operations remain registered in `src/sdk/protocol/operation-registry.ts`.
...
- Historical `BridgeClient` backend protocol surfaces, including handshake, commands, SSE, and direct host-control bypasses. The standalone `@bworx-io/worx-bridge-client` package exports only the v3 transport `SdkClient` surface.
...
The closure gate is `bun run verify:sdk-canonicalization` from this package. It is part of `bun run check`, so workspace checks and root release/prepublish checks enforce it automatically. The scanner permits only the standalone `@bworx-io/worx-bridge-client` v3 package and root `SdkClient` import; it rejects historical BridgeClient protocol surfaces, retired source trees and exports, bridge/unattended imports, executable RPC compatibility fixtures, direct authority bypasses, and unsupported machine-entrypoint import graphs.
