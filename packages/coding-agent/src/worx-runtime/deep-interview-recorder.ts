...
async function persistEnvelope(
	...
	const writeResult = await writeGuardedWorkflowEnvelopeAtomic(statePath, payload, {
		...
		policy: "source",
		expectedRevision,
		receipt: { cwd, skill: "deep-interview", owner: "worx-runtime", command, sessionId, nowIso: now },
		audit: { category: "state", verb: "write", owner: "worx-runtime", skill: "deep-interview", sessionId },
	});
...
async function syncRecorderHud(
	...
	await syncSkillActiveState({
		...
		phase,
		sessionId,
		source: "worx-runtime-deep-interview-recorder",
