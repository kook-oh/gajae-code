...
function receiptWithRequiredFields(raw: unknown, skill: CanonicalWorxWorkflowSkill): Record<string, unknown> {
	...
	receipt.skill = skill;
	if (receipt.owner !== "worx-state-cli" && receipt.owner !== "worx-runtime" && receipt.owner !== "worx-hook") {
 ...
 * the only path allowed to write normalized upgrades back to `.worx/state/**`.
 */
