...
import { getConfigRootDir } from "@bworx-io/worx-utils";
...
export async function appendLedger(
	...
	await appendJsonl(paths.ledgerPath, entry, {
		cwd,
		audit: { category: "ledger", verb: "append", owner: "worx-runtime", sessionId: resolvedSessionId },
	});
...
export async function writePlan(cwd: string, plan: UltragoalPlan, sessionId?: string | null): Promise<void> {
	...
	await writeArtifact(paths.briefPath, `${plan.brief.trim()}\n`, {
		cwd,
		audit: { category: "artifact", verb: "write", owner: "worx-runtime", sessionId: resolvedSessionId },
	});
	await writeGuardedJsonAtomic(paths.goalsPath, plan, {
		...
		audit: { category: "state", verb: "write", owner: "worx-runtime", sessionId: resolvedSessionId },
	});
...
export function categorizeComputerChangePath(value: string): UltragoalChangeCategory {
	...
	if (
		...
		normalized === "packages/coding-agent/src/defaults/worx/skills/ultragoal/SKILL.md" ||
