...
async function writeJsonAtomic(
	...
	const writeResult = await writeGuardedWorkflowEnvelopeAtomic(filePath, value, {
		...
		audit: {
			...
			category: "state",
			verb,
			owner: options?.owner ?? "worx-state-cli",
			skill: options?.skill,
			mutationId: options?.mutationId,
 ...
 * detection. Receipts carry `owner: "worx-runtime"` and `verb: "reconcile"` so the
