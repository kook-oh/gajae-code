...
export interface StateWriterAuditContext {
	...
	verb: string;
	owner: WorkflowStateMutationOwner;
	skill?: CanonicalWorxWorkflowSkill | string;
	mutationId?: string;
	fromPhase?: string;
	toPhase?: string;
	forced?: boolean;
...
export interface WorkflowTransactionJournal {
	...
	created_at: string;
	updated_at: string;
	caller?: CanonicalWorxWorkflowSkill;
	callee?: CanonicalWorxWorkflowSkill;
	paths: string[];
	steps: string[];
}
...
function resolveWorxTarget(targetPath: string, cwd = process.cwd()): string {
	if (!targetPath.trim()) throw new Error("targetPath is required");
	const projectRoot = path.resolve(cwd);
	const worxRoot = path.join(projectRoot, ".worx");
	const resolved = path.resolve(projectRoot, targetPath);
	const relative = path.relative(worxRoot, resolved);
...
export async function writeGuardedJsonAtomic(
	...
	const filePath = resolveWorxTarget(targetPath, cwdForOptions(options));
...
async function recordInvalidWorkflowTransition(args: {
	...
	try {
		await appendAuditEntry(cwd, options?.audit?.sessionId ?? "", {
			...
			owner: options?.audit?.owner ?? "worx-runtime",
...
export async function writeArtifact(
	...
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "artifact", verb: "write", owner: "worx-runtime" },
	});
}
...
export async function writeReport(targetPath: string, content: string, options?: StateWriterOptions): Promise<string> {
	return writeTextAtomic(targetPath, content, {
		...options,
		audit: options?.audit ?? { category: "report", verb: "write", owner: "worx-runtime" },
	});
}
...
export async function writeLogJsonl(targetPath: string, entry: unknown, options?: StateWriterOptions): Promise<string> {
	return appendJsonl(targetPath, entry, {
		...options,
		audit: options?.audit ?? { category: "log", verb: "append", owner: "worx-runtime" },
	});
}
...
export async function softDelete(
	...
	return updateJsonAtomic<Record<string, unknown>>(
		...
		{
			...options,
			audit: options?.audit ?? { category: "prune", verb: "soft-delete", owner: "worx-runtime" },
		},
	);
}
...
export async function hardPrune(
	...
	for (const target of targets) {
		const filePath = resolveWorxTarget(target.path, cwd);
		let stat: Stats;
		try {
...
export async function forceOverwrite(
	...
	const auditOptions = {
		...options,
		audit: options?.audit ?? { category: "force", verb: "force-overwrite", owner: "worx-state-cli", forced: true },
	};
	if (options?.raw === true) {
		const filePath = resolveWorxTarget(targetPath, cwdForOptions(options));
...
export async function appendAuditEntry(
	...
	const filePath = resolveWorxTarget(layoutAuditPath(cwd, sessionId), cwd);
	await fs.mkdir(path.dirname(filePath), { recursive: true });
	await fs.appendFile(filePath, `${JSON.stringify(entry)}\n`, "utf-8");
	return filePath;
}
