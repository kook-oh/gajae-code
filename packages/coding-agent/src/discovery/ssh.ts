 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed worx config paths and legacy root ssh.json files.
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed worx paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
