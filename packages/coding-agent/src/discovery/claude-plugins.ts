...
import { rootContainsWorxManifest } from "../extensibility/worx-plugins";
import {
...
async function listNonWorxPluginRoots(
	home: string,
	cwd: string,
	...
	for (const root of roots) {
		if (await rootContainsWorxManifest(root.path)) {
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	...
	const { roots, warnings: rootWarnings } = await listNonWorxPluginRoots(ctx.home, ctx.cwd);
