...
handoff: .worx/_session-{sessionid}/specs/deep-interview-{slug}.md
level: 3
...
- Use the active GJC workflow/state CLI as the only sanctioned writer for `.worx/` interview artifacts; do not edit `.worx/` directly without an explicit force override.
...
- Auto-mode architects are read-only: no code edits, no `.worx/` mutation, no workflow chaining, no formatters, and no execution delegation.
...
3. **Resolve threshold and source**:
   ...
   - Set these run variables exactly: `<resolvedThreshold>`, `<resolvedThresholdPercent>`, and `<resolvedThresholdSource>` (for example `WORX_CODING_AGENT_DIR/config.yml`, `$WORX_CONFIG_DIR/agent/config.yml`, `~/.worx/agent/config.yml`, `./.worx/settings.json`, `[$WORX_CONFIG_DIR|~/.worx]/settings.json`, or the selected mode default).
...
3.7. **Artifact path discipline**:
   - Final specs MUST resolve to `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly.
   - Write final specs and all ephemeral interview artifacts through the active GJC workflow/state CLI when available.
   - Direct `.worx/` file edits are forbidden unless an explicit force override is active; do not use `write`, `edit`, or `ast_edit` against `.worx/_session-{sessionid}/specs`, `.worx/_session-{sessionid}/plans`, `.worx/_session-{sessionid}/state`, or other `.worx/` paths during normal workflow operation.
   - Preferred: pass the spec markdown **inline** to the native deep-interview write command (`--write … --spec "<markdown>"`) — no scratch file is needed. The CLI is the only sanctioned writer for `.worx/_session-{sessionid}/specs`.
...
1. **Generate the specification** using opus model with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
   ...
   - Apply the self-proofread once (DIPP-5) to newly generated spec prose before persistence, including generated natural-language table cells such as coverage notes, while preserving transcript answers, quoted/source text, code identifiers, file paths, commands, JSON/settings keys, table structure/fixed labels, and `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md` unchanged.
2. **Write the final spec through the workflow CLI**: persist the artifact at `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md`
   - Always use this exact final spec path. Prefer passing the spec markdown **inline** as the `--spec` value; only when it is too large to pass inline, stage it as a file in a system temp directory (`os.tmpdir()`/`$TMPDIR`, `/tmp`, `/var/tmp`) outside the project tree and pass that path — never write scratch specs to the repo root, the project tree, or `.worx/`.
   - Use the native deep-interview write command with `--write --stage final --slug {slug} --spec <markdown-or-path> [--json]` for artifact and state persistence; direct `.worx/` file edits are forbidden unless an explicit force override is active.
...
That command persists `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md`, seeds ralplan in deliberate mode, and performs the safe deep-interview → ralplan state handoff. Skipping spec persistence leaves the Phase 5 chain blocked by design.
...
- [ ] Spec persisted to `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md` exactly via the GJC CLI (no direct `.worx/` edits without force override), covering every active topology component plus goal/constraints/acceptance criteria/clarity/ontology/transcript
...
Optional settings in `.worx/settings.json`:
...
See the Phase 5b "Approval-Gated Refinement Path" diagram for the full flow. In short: interview → spec at `.worx/_session-{sessionid}/specs/deep-interview-{slug}.md` → user selects "Refine with ralplan consensus" → `/skill:ralplan` (Planner/Architect/Critic consensus, plan written to `.worx/_session-{sessionid}/plans/`) → stop at `pending approval`. Execution is always a separate approval-gated step; deep-interview and ralplan never auto-invoke ultragoal or team just because a spec or plan exists.
