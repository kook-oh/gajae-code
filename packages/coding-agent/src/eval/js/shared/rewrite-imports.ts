...
// `__worx_import__` helper, which resolves the specifier against the active session cwd. A real
...
function buildWorxImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__worx_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__worx_import__(${sourceLiteral}, ${optionsLiteral})` : `__worx_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__worx_import__` forwards the
...
function rewriteImportNode(node: BabelImportDeclaration): string {
	...
	const importCall = buildWorxImportCall(sourceLiteral, optionsLiteral);
...
export function rewriteImports(code: string): string {
	...
	// Top-level static `import` declarations become `await __worx_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__worx_import__`
	...
	walkNodes(ast, node => {
		...
		edits.push({ start: callee.start, end: callee.end, text: "__worx_import__" });
	});
...
function returnFinalExpression(code: string): { source: string; returned: boolean } {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__worx_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__worx_set_final_expr__((expr))`
		...
		return { source: `${prefix}__worx_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
