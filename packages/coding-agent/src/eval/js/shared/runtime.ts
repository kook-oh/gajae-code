...
export interface RuntimeOptions {
	...
	/**
	 * Extra globals installed alongside `__worx_helpers__` / prelude. Use for stable, lifetime-
...
export class JsRuntime {
	...
	setCwd(cwd: string): void {
		this.#cwd = cwd;
		const session = (globalThis as { __worx_session__?: { cwd?: string } }).__worx_session__;
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		const injected: Record<string, unknown> = {
			__worx_session__: { cwd: this.#cwd, sessionId: this.sessionId },
			__worx_helpers__: this.helpers,
			__worx_call_tool__: async (name: string, args: unknown) => {
				const hooks = this.#getHooks();
				if (!hooks) throw new ToolError("Tool calls are only valid inside an active run");
				return await hooks.callTool(name, args);
			},
			__worx_import__: async (source: string, options?: ImportCallOptions) => {
			...
			__worx_emit_status__: (op: string, data: Record<string, unknown> = {}) => {
				const event: JsStatusEvent = { op, ...data };
				this.#getHooks()?.onDisplay({ type: "status", event });
			},
			__worx_log__: (level: string, ...args: unknown[]) => {
				const prefix = level === "error" ? "[error] " : level === "warn" ? "[warn] " : "";
				const text = `${prefix}${formatConsoleArgs(args)}`;
				this.#getHooks()?.onText(text.endsWith("\n") ? text : `${text}\n`);
			},
			__worx_display__: (value: unknown) => this.displayValue(value),
			__worx_set_final_expr__: (value: unknown) => {
