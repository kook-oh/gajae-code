...
if "__worx_prelude_loaded__" not in globals():
    __worx_prelude_loaded__ = True
    from pathlib import Path
    import os, json

    # __worx_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _worx_display = __worx_display  # type: ignore[name-defined]
