"""GJC Python runner — subprocess wrapper used by the coding-agent host.
...
def transform_cell(source: str) -> str:
    ...
    Rules
    -----
    * ``%name args``              -> ``__worx_magic("name", "args")``
    * ``var = %name args``        -> ``var = __worx_magic("name", "args")``
    * ``!cmd``                    -> ``__worx_shell("cmd")``
    * ``var = !cmd``              -> ``var = __worx_shell("cmd")``
    * ``%%name args\\n<body>``    -> ``__worx_magic_cell("name", "args", "<body>")``
    ...
    while i < len(lines):
        ...
        if stripped.startswith("%%"):
            ...
            body = "\n".join(body_lines)
            out.append(
                f"{indent}__worx_magic_cell({_quote_arg(name)}, {_quote_arg(args)}, {_quote_arg(body)})"
            )
            return "\n".join(out)
        ...
        if stripped.startswith("%") and not stripped.startswith("%%"):
            ...
            name, args = head
            out.append(f"{indent}__worx_magic({_quote_arg(name)}, {_quote_arg(args)})")
            i += consumed
            continue
        ...
        if stripped.startswith("!"):
            ...
            out.append(f"{indent}__worx_shell({_quote_arg(cmd)})")
            i += consumed
            continue
        ...
        if m:
            ...
            if rhs.startswith("!"):
                cmd = rhs[1:].strip()
                out.append(f"{m.group('indent')}{m.group('lhs').rstrip()} = __worx_shell({_quote_arg(cmd)})")
                i += 1
                continue
            if rhs.startswith("%") and not rhs.startswith("%%"):
                ...
                name, args = head
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __worx_magic({_quote_arg(name)}, {_quote_arg(args)})"
                )
                i += 1
                continue
...
def __worx_magic(name: str, args: str) -> Any:
...
def __worx_magic_cell(name: str, args: str, body: str) -> Any:
...
def __worx_shell(cmd: str) -> _ShellResult:
    proc = subprocess.run(
        cmd,
        shell=True,
        stdout=subprocess.PIPE,
...
def __worx_display(value: Any, *, raw: bool = False, kind: str = "display") -> None:
    if raw:
...
def _install_builtins(ns: dict) -> None:
    ns["display"] = __worx_display
    ns["__worx_display"] = __worx_display
    ns["__worx_magic"] = __worx_magic
    ns["__worx_magic_cell"] = __worx_magic_cell
    ns["__worx_shell"] = __worx_shell
...
def _exec_source(source: str, ns: dict) -> None:
    ...
    its value through ``__worx_display`` so dataframes/figures render rich.
    ...
    if isinstance(last, ast.Expr):
        ...
        if value is not None:
            __worx_display(value, kind="result")
        return
