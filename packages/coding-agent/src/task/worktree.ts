...
async function writeSyntheticTree(repoDir: string, baseTreeish: string, patches: readonly string[]): Promise<string> {
	const tempIndex = path.join(os.tmpdir(), `worx-task-index-${Snowflake.next()}`);
	try {
...
export async function commitToBranch(
	...
	const branchName = `worx/task/${taskId}`;
	...
	if (rootPatch.trim()) {
		...
		const tmpDir = path.join(os.tmpdir(), `worx-branch-${Snowflake.next()}`);
		try {
...
export async function mergeTaskBranches(
	...
	const didStash = await git.stash.push(repoRoot, "worx-task-merge");
