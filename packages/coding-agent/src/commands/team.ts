...
export default class Team extends Command {
	static description =
		"Run native GJC tmux team orchestration from inside an existing tmux/GJC --tmux session; --dry-run writes ephemeral .worx/_session-{sessionid}/state/team state only";
	static strict = false;
	...
	static flags = {
		...
		"dry-run": Flags.boolean({
			description:
				"Create ephemeral .worx/_session-{sessionid}/state/team state without starting tmux panes; do not commit generated state",
			default: false,
		}),
	};
	...
	async run(): Promise<void> {
		...
		writeText([
			...
			...(dryRun
				? [
						"dry-run: wrote ephemeral .worx/_session-{sessionid}/state/team state only; do not commit generated .worx state",
					]
				: []),
		]);
	}
