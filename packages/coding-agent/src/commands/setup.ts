...
export default class Setup extends Command {
	...
	static flags = {
		...
		"session-command": Flags.string({ description: "Explicit WORX session command for Hermes to launch" }),
		"no-worktree": Flags.boolean({ description: "Disable default WORX --worktree isolation for Hermes sessions" }),
		"worktree-name": Flags.string({ description: "Named WORX --worktree branch for Hermes sessions" }),
		...
		"worx-command": Flags.string({ description: "Command used to start `worx mcp-serve coordinator`" }),
	...
	async run(): Promise<void> {
		...
		const cmd: SetupCommandArgs = {
			...
			flags: {
				...
				worxCommand: flags["worx-command"],
				target: flags.target,
