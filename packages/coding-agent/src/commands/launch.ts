...
export async function persistCoordinatorLaunchFailure(
	...
	const stateFile = env[WORX_COORDINATOR_SESSION_STATE_FILE_ENV]?.trim();
	if (!stateFile) return;
	...
	const payload = {
		schema_version: 1,
		session_id: env[WORX_COORDINATOR_SESSION_ID_ENV]?.trim() || null,
		state: "errored",
		...
		...(env[WORX_COORDINATOR_SESSION_ID_ENV]?.trim()
			? { owner_generation: env[WORX_TMUX_OWNER_GENERATION_ENV] ?? null }
			: {}),
	};
