 ...
 *
 *     ESC ] 777 ; notify;Terax;worx;<event> BEL
 *
 ...
 * any host that wants agent status can parse the stream without worx knowing it
 * exists.
 *
 * `notify;Terax;` is the wire identifier of the only protocol that currently
 * carries per-turn agent state, not a host check - the payload names worx as the
 * reporting agent.
 *
...
function marker(event: HostStatusEvent): string {
	return `\x1b]777;notify;Terax;worx;${event}\x07`;
}
