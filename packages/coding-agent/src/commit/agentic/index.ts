...
export async function runAgenticCommit(args: CommitCommandArgs): Promise<void> {
	...
	const forceFallback = $pickenv("WORX_COMMIT_TEST_FALLBACK", "PI_COMMIT_TEST_FALLBACK")?.toLowerCase() === "true";
	if (forceFallback) {
	...
	if (!usedFallback && !commitState.proposal && !commitState.splitProposal) {
		if ($pickenv("WORX_COMMIT_NO_FALLBACK", "PI_COMMIT_NO_FALLBACK")?.toLowerCase() !== "true") {
