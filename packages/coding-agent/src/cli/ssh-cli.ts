 ...
 *
 * Handles `worx ssh <command>` subcommands for SSH host configuration management.
 */

import { getSSHConfigPath } from "@bworx-io/worx-utils";
...
async function handleAdd(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!name) {
		...
		process.stdout.write(
			chalk.dim("Usage: worx ssh add <name> --host <address> [--user <user>] [--port <port>] [--key <path>]\n"),
		);
		process.exitCode = 1;
		return;
	}
	...
	if (!host) {
		...
		process.stdout.write(chalk.dim("Usage: worx ssh add <name> --host <address>\n"));
		process.exitCode = 1;
		return;
	}
...
async function handleRemove(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!name) {
		process.stdout.write(chalk.red("Error: Host name required\n"));
		process.stdout.write(chalk.dim("Usage: worx ssh remove <name> [--scope project|user]\n"));
		process.exitCode = 1;
		return;
	}
...
async function handleList(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!hasProject && !hasUser) {
		...
		process.stdout.write(chalk.dim("Add one with: worx ssh add <name> --host <address>\n"));
		return;
	}

	if (hasProject) {
		process.stdout.write(chalk.bold("Project SSH Hosts (.worx/ssh.json):\n"));
	...
	if (hasUser) {
		process.stdout.write(chalk.bold("User SSH Hosts (~/.worx/agent/ssh.json):\n"));
