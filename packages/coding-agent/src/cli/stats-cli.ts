 ...
 *
 * Handles `worx stats` subcommand for viewing AI usage statistics.
 */

import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@bworx-io/worx-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@bworx-io/worx-stats");
	const stats = await getDashboardStats();
