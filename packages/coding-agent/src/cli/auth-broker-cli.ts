/**
 * `worx auth-broker` command handlers.
 *
 * Sub-verbs:
...
import { $which, APP_NAME, getAgentDbPath, getConfigRootDir, isEnoent, logger, VERSION } from "@bworx-io/worx-utils";
import { $ } from "bun";
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// skip the rotating ~/.worx/logs/ file the TUI default would have used.
...
async function runLogin(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!providerArg) {
		throw new Error("Usage: worx auth-broker login <provider> [--via=user@host]");
	}
...
async function runLocalLogin(provider: OAuthProvider): Promise<void> {
	...
	const piAiCli = Bun.fileURLToPath(import.meta.resolve("@bworx-io/worx-ai/cli"));
...
async function runLogout(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	const providerArg = flags.provider;
	if (!providerArg) {
		throw new Error("Usage: worx auth-broker logout <provider>");
	}
...
/**
 * Maps the `type` field of a CLIProxyAPI credential JSON to the worx provider id.
...
async function loadImportPlan(
	...
	for (const file of files) {
		...
		if (!provider) {
			skipped.push({
				file,
				reason: `cannot determine worx provider from type=${json.type ?? "?"} (pass --provider to override)`,
			});
			continue;
		}
...
async function runImport(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!target) {
		throw new Error("Usage: worx auth-broker import <file|dir> [--provider=<id>] [--include-disabled] [--dry-run]");
	}
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"WORX_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
	if (flags.fromLocal !== true) {
		throw new Error(
			"`worx auth-broker migrate` requires an explicit source. Pass `--from-local` to migrate from the local SQLite store and env vars.",
		);
	}
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set WORX_AUTH_BROKER_URL to enable).";
