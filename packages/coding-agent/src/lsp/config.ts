...
import { $which, isRecord, logger, pathIsWithin } from "@bworx-io/worx-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.worx/, .gemini/)
	...
	// User config directories (~/.worx/agent/, ~/.gemini/)
 ...
 * 2. Project config dirs: .worx/lsp.*, .gemini/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.worx/agent/lsp.*, ~/.gemini/lsp.* (+ hidden variants)
