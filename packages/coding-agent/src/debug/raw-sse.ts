import { type Component, matchesKey, padding, replaceTabs, truncateToWidth, visibleWidth } from "@bworx-io/worx-tui";
	...
	#renderRawLines(innerWidth: number): string[] {
		...
		if (snapshot.droppedRecords > 0) {
			lines.push(
				theme.fg(
					"warning",
					`: worx-debug-dropped records=${snapshot.droppedRecords} chars=${snapshot.droppedChars}`,
				),
			);
			lines.push("");
		}
		for (const record of snapshot.records) {
			...
			if (record.kind === "event" && record.truncated) {
				lines.push(theme.fg("warning", `: worx-debug-event-truncated originalChars=${record.originalChars}`));
			}
			lines.push("");
		}
		return lines;
