...
const SIXEL_PLACEHOLDER_PREFIX = "__WORX_SIXEL_SEQUENCE_";
...
export function isSixelPassthroughEnabled(): boolean {
	const forcedProtocol = $pickenv("WORX_FORCE_IMAGE_PROTOCOL", "PI_FORCE_IMAGE_PROTOCOL")?.trim().toLowerCase();
	return forcedProtocol === "sixel" && $pickflag("WORX_ALLOW_SIXEL_PASSTHROUGH", "PI_ALLOW_SIXEL_PASSTHROUGH");
}
...
export function sanitizeWithOptionalSixelPassthrough(text: string, sanitize: (text: string) => string): string {
	...
	return sanitized.replace(/__WORX_SIXEL_SEQUENCE_(\d+)__/gu, (_, indexText: string) => {
