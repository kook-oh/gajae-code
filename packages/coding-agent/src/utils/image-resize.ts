...
/**
 * Read `WORX_NO_WEBP` per-call so runtime toggles take effect.
...
function isWebPExcluded(): boolean {
	const raw = Bun.env.WORX_NO_WEBP;
 ...
 *
 * Set WORX_NO_WEBP to exclude WebP from encoding (llama.cpp STB doesn't decode it).
 *
...
export async function resizeImageBuffer(
	...
	try {
		...
		// WebP usually beats JPEG by 25–35% but is disabled when WORX_NO_WEBP is set
		...
		// unless WORX_NO_WEBP is set (llama.cpp STB incompatibility).
