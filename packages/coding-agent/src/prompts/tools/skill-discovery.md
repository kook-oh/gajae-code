...
<instruction>
- Searches only custom runtime skill locations: nearest project `.worx/skills`; then, under the home directory, canonical `<config>/agent/skills`, configured legacy `<config>/skills`, and historical legacy `.worx/skills`. `<config>` is the home-relative directory name from `WORX_CONFIG_DIR`, then `PI_CONFIG_DIR`, then `.worx`; even an absolute-looking configured name is joined beneath `<home>`. Duplicate names use that exact precedence. Built-in, bundled, and internal workflow skills are intentionally excluded.
