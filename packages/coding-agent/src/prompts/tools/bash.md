...
- `{{this}}`
{{/each}}
Use it only for sanctioned GJC workflow CLI persistence or state read/write/contract operations; the only per-command env override allowed is `WORX_RALPLAN_ARTIFACT` when paired with `gjc ralplan --write … --artifact-env WORX_RALPLAN_ARTIFACT`, and all other shell command shapes are blocked before execution.
{{/when}}
</restricted-bash-mode>
{{/if}}
