...
```text
pi-mono → oh-my-pi → gajae-code → byteWORX worx-code
```

The MIT license and copyright notices for Mario Zechner (pi-mono), Can Bölük
(oh-my-pi), Yeachan-Heo and Gajae Code Contributors (gajae-code), and
byteWORX (worx-code) are retained in the root `LICENSE`.
