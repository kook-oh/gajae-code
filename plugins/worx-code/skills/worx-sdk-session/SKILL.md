...
description: Operate GJC SDK sessions from the CLI (`worx sdk session list|inspect|send|status|tail|elevate|raw` plus the explicit raw control|query|global hatch). Advisory reference: broker-bound, credential-free output; mutating verbs run only when explicitly invoked.
---
...
`worx sdk session raw control|query|global` dispatches one SDK operation with `--op`
