...
run_post_code() {
  ...
  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" WORX_ISSUE1938_TMUX_SOCKET="$RECOVERY_SOCKET" WORX_BIN="$product_bin" WORX_SESSION_TMUX_BIN="$tmux_wrapper" WORX_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" WORX_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/worx-session/create.sh" "$recovery_session" "$WORKTREE"; then append_case unexpected_incident_recovery failed "$started" "$(now)" '' "$recovery_session" '' SIGTERM unexpected_owner_loss UNKNOWN launch_failed; return 1; fi
  ...
  if ! run_unsafe_service env TMUX_TMPDIR="$WORKTREE/tmux" WORX_ISSUE1938_TMUX_SOCKET="$RECOVERY_SOCKET" WORX_BIN="$product_bin" WORX_SESSION_TMUX_BIN="$tmux_wrapper" WORX_SESSION_MONITOR_INTERVAL="$MONITOR_INTERVAL_SECONDS" WORX_SESSION_SKIP_ROUTER=1 bash "$WORKTREE/scripts/worx-session/create.sh" "$recovery_session" "$WORKTREE"; then append_case unexpected_incident_recovery failed "$started" "$(now)" '' "$recovery_session" '' SIGTERM unexpected_owner_loss UNKNOWN recovery_launch_failed '' '' "$recovery_key" "$recovery_latency"; return 1; fi
