...
async function materialize(): Promise<{ files: Map<string, string>; root: string }> {
	...
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "worx-sdk-skills-test-"));
	roots.push(root);
...
async function endpointRepo(url: string, token: string): Promise<string> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "worx-sdk-template-test-"));
	roots.push(root);
...
async function runTypeScriptTemplate(
	...
	const child = Bun.spawn(["bun", path.join(repoRoot, "sdk-skills", "worx-sdk-author", "templates", "direct-sdk.ts"), ...args], {
		stdin: "pipe",
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("generated external GJC SDK skills", () => {
	it("renders the exact namespaced six-file versioned contract", () => {
		...
		expect([...files.keys()].sort()).toEqual(
			[
				BUNDLE_MANIFEST_NAME,
				"worx-sdk-discover/SKILL.md",
				"worx-sdk-operate/SKILL.md",
				"worx-sdk-author/SKILL.md",
				"worx-sdk-author/templates/direct-sdk.ts",
				"worx-sdk-author/templates/direct-sdk.py",
			].sort(),
		);
		...
		expect(manifest.bundle).toBe("worx-sdk-skills");
		expect(manifest.formatVersion).toBe(BUNDLE_FORMAT_VERSION);
		expect(manifest.files).toEqual(bundleContentFiles(files));
		expect(files.get("worx-sdk-discover/SKILL.md")).toContain("name: worx-sdk-discover");
		expect(files.get("worx-sdk-operate/SKILL.md")).toContain("name: worx-sdk-operate");
		expect(files.get("worx-sdk-author/SKILL.md")).toContain("name: worx-sdk-author");
	});
	...
	it("keeps the four default workflow skills closed and adds no extra skills", async () => {
		...
		expect(skills).toEqual(["worx-sdk-author/SKILL.md", "worx-sdk-discover/SKILL.md", "worx-sdk-operate/SKILL.md"]);
		for (const key of files.keys()) {
			...
			if (topLevel !== BUNDLE_MANIFEST_NAME) {
				expect(["worx-sdk-author", "worx-sdk-discover", "worx-sdk-operate"]).toContain(topLevel);
			}
		}
		...
		await fs.mkdir(path.join(extra.root, "worx-sdk-bash"));
		await Bun.write(path.join(extra.root, "worx-sdk-bash", "SKILL.md"), "---\nname: worx-sdk-bash\n---\n");
		expect(await findUnexpectedSdkSkillFiles(extra.files, extra.root)).toEqual([path.join("worx-sdk-bash", "SKILL.md")]);
	...
	it("fails closed on missing, malformed, and unsupported manifest versions", async () => {
		...
		expect(
			validateBundleManifest(JSON.stringify({ bundle: "worx-sdk-skills", formatVersion: 999, files: contentFiles }), contentFiles),
		).toContain("unsupported");
		expect(
			validateBundleManifest(JSON.stringify({ bundle: "worx-sdk-skills", formatVersion: BUNDLE_FORMAT_VERSION, files: ["worx-sdk-bash/SKILL.md"] }), contentFiles),
	...
	it("upgrades installed v1 bundles and fails closed on legacy or future layouts", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({ bundle: "worx-sdk-skills", formatVersion: 2, files: contentFiles }, null, 2) + "\n",
		);
	...
	it("passes the generator drift check as a subprocess", async () => {
		const child = Bun.spawn(["bun", path.join(repoRoot, "scripts", "generate-worx-sdk-skills.ts"), "--check"], {
			stdout: "pipe",
			stderr: "pipe",
		});
		const [exitCode, stdout, stderr] = await Promise.all([
	...
	it("rejects missing, drifted, and unexpected generated files", async () => {
		...
		await fs.rm(path.join(missing.root, "worx-sdk-discover", "SKILL.md"));
		expect(await checkSdkSkillFiles(missing.files, missing.root, false)).toBe(1);

		const drifted = await materialize();
		await Bun.write(path.join(drifted.root, "worx-sdk-operate", "SKILL.md"), `${drifted.files.get(path.join("worx-sdk-operate", "SKILL.md")) ?? ""}drift\n`);
		expect(await checkSdkSkillFiles(drifted.files, drifted.root, false)).toBe(1);

		const symlinked = await materialize();
		const target = path.join(symlinked.root, "worx-sdk-operate", "SKILL.md");
		...
		if (process.platform !== "win32") {
			...
			const fifoPath = path.join(fifo.root, "worx-sdk-discover", "SKILL.md");
	...
	it("keeps direct-client templates fail-closed and credential-safe", () => {
		const files = renderSdkSkillFiles();
		const typescript = files.get("worx-sdk-author/templates/direct-sdk.ts") ?? "";
		const python = files.get("worx-sdk-author/templates/direct-sdk.py") ?? "";
