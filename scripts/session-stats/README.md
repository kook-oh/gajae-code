...
(`~/.worx/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.worx/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py       # walks ~/.worx/agent/sessions/ and populates ss_* tables
