...
function usage(): never {
	console.error("Usage: bun scripts/generate-worx-workflow-manifest.ts [--write|--check]");
	process.exit(2);
}
...
function main(): void {
	...
	console.error(
		`G3 DRIFT: ${path.relative(repoRoot, GENERATED_MANIFEST)} does not match serializeManifestProjection(). Run bun scripts/generate-worx-workflow-manifest.ts --write.`,
	);
	process.exit(1);
}
