...
This document explains how token/tool streaming is normalized in `@bworx-io/worx-ai`, then propagated through `@bworx-io/worx-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@bworx-io/worx-ai`
