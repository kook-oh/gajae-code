...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/gajae-code` → `@bworx-io/worx-code`
  - `@mariozechner/pi-agent-core` → `@bworx-io/worx-agent-core`
  - `@mariozechner/pi-tui` → `@bworx-io/worx-tui`
  - `@mariozechner/pi-ai` → `@bworx-io/worx-ai`
...
- In `packages/coding-agent`, use `logger` from `@bworx-io/worx-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@bworx-io/worx-code-natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@bworx-io/worx-code-natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field       | `pkg.worx` preferred; fallback to `pkg.pi` remains |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@bworx-io/worx-code-natives`
