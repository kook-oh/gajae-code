...
- user-level: `$HOME/${WORX_CONFIG_DIR:-.worx}/agent/extensions`
- project-level: `<cwd>/.worx/extensions`
...
`WORX_CONFIG_DIR` selects the **home-relative** config directory name: the config root is `<home>/<WORX_CONFIG_DIR>`, defaulting to `~/.worx`. It does not select a project directory — the project-level path is the constant `.worx` (`discovery/helpers.ts`, `getProjectAgentDir()`), so `WORX_CONFIG_DIR` never moves it. `WORX_CODING_AGENT_DIR` overrides the agent directory **path** rather than naming one under `$HOME`; it is resolved with `path.resolve`, so an absolute value is used as-is and a relative value is resolved against the current working directory.
