...
| Level   | Path                       | Purpose                     |
| ------- | -------------------------- | --------------------------- |
| Global  | `~/.worx/agent/secrets.yml` | Plain and regex secrets across all projects |
| Project | `<cwd>/.worx/secrets.yml`   | Project-specific plain secrets              |

Project plain entries override global plain entries with matching `content`; a global regex with the same `content` remains active. Project-scope regex entries are ignored because workspace-contained files are not trusted to supply executable regex patterns. This project scope includes `<cwd>/.worx/secrets.yml` and any caller-supplied agent directory whose lexical or canonical path is contained within the workspace.
...
Regex entries are supported only by agent configuration outside the current workspace (normally `~/.worx/agent/secrets.yml`). Use `type: plain` for workspace-contained configuration.
