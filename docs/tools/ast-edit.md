...
2. The wrapper reads `WORX_MAX_AST_FILES` via `$envpos(..., 1000)` and uses that as the native `maxFiles` cap for both preview and apply.
...
## Limits & Caps
- File cap exposed by the wrapper: `WORX_MAX_AST_FILES`, default `1000`, in `packages/coding-agent/src/tools/ast-edit.ts`.
