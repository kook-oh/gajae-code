...
- Worktree path is `path.join(getWorktreesDir(), encodeRepoPathForFilesystem(primaryRepoRoot), localBranch)`, where `getWorktreesDir()` is `~/.worx/wt`; effective path is `~/.worx/wt/<encoded-primary-repo-root>/pr-<number>`.
...
- The branch push metadata is persisted with `git config` under the repository's shared `.git/config` as:
  ...
  - `branch.pr-<number>.worxPrHeadRef`
  - `branch.pr-<number>.worxPrUrl`
  - `branch.pr-<number>.worxPrIsCrossRepository`
  - `branch.pr-<number>.worxPrMaintainerCanModify`
...
- Filesystem
  ...
  - `pr_checkout` may create directories under `~/.worx/wt/<encoded-primary-repo-root>/` and add git worktrees there.
