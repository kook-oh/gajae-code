...
3. `getSSHConfigPath("project")` and `getSSHConfigPath("user")` in `packages/utils/src/dirs.ts` resolve those managed files to `.worx/ssh.json` in the project and `~/.worx/agent/ssh.json` in the user config dir. This tool does not read `~/.ssh/config`.
...
- Subprocesses / native bindings
  ...
  - Sanitizes streamed text with `@bworx-io/worx-code-natives` text sanitization.
