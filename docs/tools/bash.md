...
| `pty` | `boolean` | No | Request PTY mode. Default `false`. PTY is used only when `pty: true`, `WORX_NO_PTY !== "1"`, and the tool context has a UI. |
...
2. Foreground PTY
   - Requires `pty: true`, UI context, and `WORX_NO_PTY !== "1"`.
...
- Subprocesses / native bindings
  - Non-PTY uses native shell execution via `@bworx-io/worx-code-natives` (`Shell.run()` or `executeShell()`).
...
- PTY mode is ignored in non-UI contexts and when `WORX_NO_PTY=1`; the tool silently falls back to non-PTY execution.
