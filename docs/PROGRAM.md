...
| **T2. AX 플랫폼** | Temporal 기반 워크플로 엔진 + Review Hub + 칸반 + 설계·문서(SSOT) + worx-code 에이전트 연동 | **`bworx-io/worx-ax` 레포 `docs/`** — ARCHITECTURE.md(아키텍처·결정·메뉴·워크플로·로드맵) | 계획 확정(v0.7), Phase 0 대기 — **엔진 툴명 확정으로 Phase 2 선행 의존 해소됨** |
