...
Keys are used exactly as written. A `PI_`-prefixed key in a `.env` file is not mirrored to its `WORX_` counterpart, or the reverse — where both spellings are accepted it is because the reading code asks for both names.
...
`gjc ralplan`, `gjc deep-interview`, and `gjc state` are private runtime bridge commands. They require `WORX_RUNTIME_BINARY` (or legacy `WORX_LEGACY_RUNTIME_BINARY`) to point at the private runtime executable; public bundled workflow use remains through `/skill:ralplan` and `/skill:deep-interview` inside a GJC session.

| Variable | Behavior |
| --- | --- |
| `WORX_RUNTIME_BINARY` | Private runtime bridge binary for `gjc ralplan`, `gjc deep-interview`, and `gjc state` |
| `WORX_LEGACY_RUNTIME_BINARY` | Legacy fallback bridge binary name |
...
| Variable | Behavior |
| --- | --- |
| `WORX_LAUNCH_POLICY` | Launch policy for `--tmux` startup: `tmux` (default) or `direct` (skip the tmux session) |
| `WORX_TMUX_SESSION` | Explicit tmux session name override for `--tmux` startup. Use a unique value (for example `WORX_TMUX_SESSION=gjc-fresh-$(date +%s) gjc --tmux`) to force a fresh named session. |
| `WORX_TMUX_COMMAND` | tmux binary/name override for every GJC tmux flow (`WORX_TEAM_TMUX_COMMAND` is honored as a team-path alias). This is not a shell command line; include only the executable path/name, not flags. |
| `WORX_TMUX_PROFILE` | Set `0`/`false`/`off` to apply only the required ownership tags and skip the scroll/mouse/clipboard profile |
| `WORX_MOUSE` | Set `0`/`false`/`off` to skip the managed profile's tmux `mouse on`; this does not disable GJC's own mouse support |
| `WORX_PSMUX_COMMAND` | Identifies a psmux wrapper for Windows alias resolution. The value must resolve to the same executable identity as the selected `tmux` command; unresolved or conflicting evidence fails closed. |
| `WORX_PSMUX_DETECTION` | Set `0`/`false`/`off` to skip banner-based psmux detection. Executable-name and alias-identity safety checks still apply. |
| `WORX_PSMUX_FORCE_DETECT` | Set `1`/`true`/`on` to re-probe the multiplexer on every call instead of caching the per-process verdict. |
...
GJC does not expose a `WORX_TMUX_NAMESPACE` runtime knob or parse flags from `WORX_TMUX_COMMAND`. Do not set `WORX_TMUX_COMMAND="psmux -L my-project"` and do not recover with ambient `tmux`/`psmux` or a manually supplied `-L` value; `WORX_TMUX_COMMAND` is one executable path/name. Use the GJC session or lifecycle operation so it reuses the persisted ProviderAuthority. If that authority cannot be read and re-proved, GJC refuses the operation.
...
| `WORX_COORDINATOR_MCP_SETUP_MANAGED_BY` | Marker written by `gjc setup hermes` for safe managed config updates. |
| `WORX_COORDINATOR_MCP_SETUP_SCHEMA_VERSION` | Managed setup schema version written by `gjc setup hermes`. |
| `WORX_COORDINATOR_MCP_SETUP_SIGNATURE` | Deterministic managed setup signature used to detect safe updates versus unmanaged conflicts. |
...
| Variable                   | Default / behavior                                              |
| -------------------------- | --------------------------------------------------------------- |
| `WORX_AI_GEMINI_CLI_VERSION` | Overrides Gemini CLI user-agent version tag (`0.49.0` if unset). `PI_AI_GEMINI_CLI_VERSION` remains supported as a legacy fallback. |

### OpenAI code provider responses (feature/debug controls)

| Variable                             | Behavior                                             |
| ------------------------------------ | ---------------------------------------------------- |
| `WORX_OPENAI_CODE_DEBUG`                     | `1`/`true` enables OpenAI code provider debug logging      |
| `WORX_NO_STRICT`                       | Global bypass for OpenAI-style strict schema enforcement (`adaptSchemaForStrict`); legacy alias `PI_NO_STRICT` |
| `WORX_OPENAI_CODE_WEBSOCKET`                 | `1`/`true` enables websocket transport preference    |
| `WORX_OPENAI_CODE_WEBSOCKET_V2`              | `1`/`true` enables websocket v2 path                 |
| `WORX_OPENAI_CODE_WEBSOCKET_IDLE_TIMEOUT_MS` | Positive integer override (default 300000)           |
| `WORX_OPENAI_CODE_WEBSOCKET_RETRY_BUDGET`    | Non-negative integer override (default 5)            |
| `WORX_OPENAI_CODE_WEBSOCKET_RETRY_DELAY_MS`  | Positive integer base backoff override (default 500) |
| `WORX_OPENAI_STREAM_IDLE_TIMEOUT_MS`   | Positive integer OpenAI stream idle timeout override. Unset: 120s, except xAI Grok and Grok Build use 300s (same floor as Anthropic long-reasoning). `0` disables. |
...
| `WORX_OPENAI_CODE_WEB_SEARCH_MODEL`                         | OpenAI code search provider model override                          |
...
| Variable            | Behavior                                                                        |
| ------------------- | ------------------------------------------------------------------------------- |
| `WORX_AUTH_NO_BORROW` | If set, disables macOS native-app token borrowing path in Perplexity login flow |
...
| Variable                  | Default / behavior                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `WORX_PY`                   | Eval backend override: `0`/`bash`=JavaScript only, `1`/`py`=Python only, `mix`/`both`=both; invalid values ignored |
| `WORX_PYTHON_SKIP_CHECK`    | If `1`, skips Python interpreter availability checks (subprocess runner still starts on demand)                     |
| `WORX_PYTHON_INTEGRATION`   | If `1`, opts gated integration tests in (e.g. `python-runner.integration.test.ts`) into running against real Python |
| `WORX_PYTHON_IPC_TRACE`     | If `1`, logs NDJSON frames exchanged with the Python runner subprocess                                              |
...
- Python env filtering denies common API keys and allows safe base vars + `LC_`, `XDG_`, `WORX_` prefixes.
...
| Variable                     | Default / behavior                                                                                 |
| ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `WORX_SMOL_MODEL`              | Ephemeral model-role override for `smol` (CLI `--smol` takes precedence)                           |
| `WORX_SLOW_MODEL`              | Ephemeral model-role override for `slow` (CLI `--slow` takes precedence)                           |
| `WORX_PLAN_MODEL`              | Ephemeral model-role override for `plan` (CLI `--plan` takes precedence)                           |
| `WORX_NO_TITLE`                | If set (any non-empty value), disables auto session title generation on first user message         |
| `WORX_NO_CMUX_RENAME`         | If set (any non-empty value), disables renaming the containing cmux workspace to the current session name |
| `NULL_PROMPT`                | If `true`, system prompt builder returns empty string                                              |
| `WORX_BLOCKED_AGENT`           | Blocks a specific subagent type in task tool                                                       |
| `WORX_SUBPROCESS_CMD`          | Overrides subagent spawn command (`gjc` / `gjc.cmd` resolution bypass)                             |
| `WORX_TASK_MAX_OUTPUT_BYTES`   | Max captured output bytes per subagent (default `500000`)                                          |
| `WORX_TASK_MAX_OUTPUT_LINES`   | Max captured output lines per subagent (default `5000`)                                            |
| `WORX_TIMING`                  | If set (any non-empty value), prints a hierarchical timing-span tree to **stderr** via `logger.printTimings()`. In interactive mode the tree prints once the agent is ready (before the TUI starts); in print mode it prints after the whole prompt batch completes. Print-mode prompts are wrapped in `print:prompt:initial` / `print:prompt:next` spans so each user message shows up as its own row. `WORX_TIMING=x` exits the process with code 0 right after printing in interactive mode (use to measure cold startup only). `WORX_TIMING=full` lists every module-load entry instead of just the top N. |
| `WORX_PACKAGE_DIR`             | Overrides package asset base dir resolution (docs/examples/changelog path lookup)                  |
| `WORX_DISABLE_LSPMUX`          | Canonical lspmux opt-out. A truthy value disables lspmux probing and wrapping; `PI_DISABLE_LSPMUX` is a supported compatibility alias with the same effect. |
| `PI_DISABLE_LSPMUX`           | Supported compatibility alias for `WORX_DISABLE_LSPMUX`; a truthy value also disables lspmux probing and wrapping. |
...
| `WORX_EDIT_VARIANT`            | Forces edit tool variant (`patch`, `replace`, `hashline`, `vim`, `apply_patch`). The force beats `edit.modelVariants`, `edit.mode`, and automatic model-family routing; invalid values fail fast at startup. `PI_EDIT_VARIANT` is the legacy alias. |
| `WORX_FORCE_IMAGE_PROTOCOL`    | Forces supported image protocol (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) where used            |
| `WORX_ALLOW_SIXEL_PASSTHROUGH` | Allows SIXEL passthrough when `WORX_FORCE_IMAGE_PROTOCOL=sixel`                                      |
...
| Variable                   | Behavior                                                                       |
| -------------------------- | ------------------------------------------------------------------------------ |
| `WORX_BASH_NO_CI`            | Suppresses automatic `CI=true` injection into spawned shell env                |
| `PI_BASH_NO_CI`             | Legacy alias fallback for `WORX_BASH_NO_CI`                                     |
| `CLAUDE_BASH_NO_CI`         | Legacy alias fallback for `WORX_BASH_NO_CI`                                     |
| `WORX_BASH_NO_LOGIN`         | Disables login-shell mode; shell args become `['-c']` instead of `['-l','-c']` |
| `PI_BASH_NO_LOGIN`          | Legacy alias fallback for `WORX_BASH_NO_LOGIN`                                  |
| `CLAUDE_BASH_NO_LOGIN`      | Legacy alias fallback for `WORX_BASH_NO_LOGIN`                                  |
...
Current implementation: `WORX_BASH_NO_CI` and `WORX_BASH_NO_LOGIN` are resolved first, then the `PI_*` and `CLAUDE_*` aliases above. Both are boolean-like: only `1`/`Y`/`TRUE`/`YES`/`ON` (case-insensitive) enable them, so an explicit `WORX_BASH_NO_LOGIN=0` keeps the login shell even when a legacy alias is truthy. The shell prefix is read from `PI_SHELL_PREFIX`/`CLAUDE_CODE_SHELL_PREFIX` only; `WORX_SHELL_PREFIX` is not currently honored.
...
| Variable                  | Behavior                                                                              |
| ------------------------- | ------------------------------------------------------------------------------------- |
| `WORX_NOTIFICATIONS`       | `0` is a hard notification runtime opt-out; `1` explicitly enables the generic current-session path even without a globally configured adapter. |
| `WORX_NOTIFICATIONS_TOKEN` | An explicit generic current-session opt-in token. It has the same runtime precedence as `WORX_NOTIFICATIONS=1`; it does not supply or override global Telegram credentials. |
| `WORX_NOTIFICATIONS_STREAM` | `1` forces live assistant-output streaming for this process; `0` / `off` / `false` disables it. Unset or unknown values defer to the global `notifications.telegram.streaming.enabled` preference, which defaults to `true` and activates durable streaming only for a configured Telegram adapter. |
| `WORX_NOTIFICATIONS_STREAM_INTERVAL_MS` | Minimum interval between live Telegram stream edits; defaults to `500` and clamps to at least `200`. |
| `WORX_NOTIFICATIONS_TURN_MAX` | Optional finalized turn-text cap for notification streaming; defaults to the bounded full-turn ceiling for split-capable clients. |
| `WORX_NOTIFY`              | `off` / `0` / `false` suppresses the notification control surface for this process, including completion notifications; global config is untouched and child processes inherit it. It wins over explicit notification opt-in. Use it for non-interactive runs (`gjc -p --no-session`) that must remain silent. |
| `WORX_TUI_WRITE_LOG`        | If set, logs TUI writes to file                                                       |
| `WORX_HARDWARE_CURSOR`      | If `1`, enables hardware cursor mode                                                  |
| `WORX_CLEAR_ON_SHRINK`      | If `1`, clears empty rows when content shrinks                                        |
| `WORX_DEBUG_REDRAW`         | If `1`, enables redraw debug logging                                                  |
| `WORX_TUI_DEBUG`            | If `1`, enables deep TUI debug dump path                                              |
| `WORX_FORCE_IMAGE_PROTOCOL` | Forces terminal image protocol detection (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) |
| `WORX_TUI_KEYBOARD_PROTOCOL` | Enhanced keyboard input (Kitty keyboard protocol + xterm modifyOtherKeys). Enabled by default; set `0` / `false` to leave the keyboard in its default mode. Use this when a terminal (e.g. Android Termius) breaks IME/Hangul composition while these enhanced modes are active. |
| `WORX_TUI_SYNCHRONIZED_OUTPUT` | Synchronized-output framing (`CSI ?2026h/l`) is enabled by default. Set `0` / `false` / `off` / `no` before starting or restarting GJC to remove that framing for terminal parsers that render it incorrectly. This is a process-wide compatibility and diagnostic switch, not tmux/Byobu client detection or per-client negotiation. Disabling it may expose visible tearing; return to the default after diagnosis unless the client requires the workaround. |
...
| Variable                  | Behavior                                                            |
| ------------------------- | ------------------------------------------------------------------- |
| `WORX_COMMIT_TEST_FALLBACK` | If `true` (case-insensitive), force commit fallback generation path |
| `WORX_COMMIT_NO_FALLBACK`   | If `true`, disables fallback when agent returns no proposal         |
| `WORX_COMMIT_MAP_REDUCE`    | If `false`, disables map-reduce commit analysis path                |
...
`--mode rpc`, `--mode rpc-ui`, and `--mode bridge` have been removed. The retired bridge-prefixed variables and `WORX_RPC_EMIT_TITLE` are not runtime configuration variables. Use the [SDK machine interface](./sdk.md) for external machine control.
